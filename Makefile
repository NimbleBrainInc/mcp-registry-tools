...
BUNDLE_NAME = mcp-registry-tools
VERSION ?= 0.0.1
...
test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/mcp_registry_tools --cov-report=term-missing
...
run-stdio: ## Run in stdio mode (for Claude desktop)
	uv run fastmcp run src/mcp_registry_tools/server.py

run-http: ## Run HTTP server with uvicorn
	uv run uvicorn mcp_registry_tools.server:app --host 0.0.0.0 --port 8000
...
bump: ## Bump version across all files (usage: make bump VERSION=0.2.0)
	...
	@sed -i '' 's/^__version__ = ".*"/__version__ = "$(VERSION)"/' src/mcp_registry_tools/__init__.py
	@echo "Updated:"
	@echo "  manifest.json:                        $$(jq -r .version manifest.json)"
	@echo "  server.json:                          $$(jq -r .version server.json)"
	@echo "  pyproject.toml:                       $$(grep '^version' pyproject.toml)"
	@echo "  src/mcp_registry_tools/__init__.py:   $$(grep '__version__' src/mcp_registry_tools/__init__.py)"
