...
Builds the registry-tools bundle, deploys it in a Docker container,
...
def mcpb_container(bundle_server, bundle_path):
    ...
    container = (
        ...
        .with_env("REGISTRY_URL", "https://registry.mpak.dev")
...
def test_health_endpoint(mcpb_container):
    ...
    assert data["service"] == "mcp-registry-tools"


@pytest.mark.asyncio
async def test_mcp_tools_list(mcpb_container):
    """Test that the MCP tools/list endpoint returns both registry tools."""
    base_url = mcpb_container
    ...
    async with streamablehttp_client(f"{base_url}/mcp") as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            assert expected_tools == tool_names, f"Unexpected tools: {tool_names}"
...
@pytest.mark.asyncio
async def test_mcp_registry_search(mcpb_container):
    """Test registry_search against the live mpak registry."""
    base_url = mcpb_container
    ...
    async with streamablehttp_client(f"{base_url}/mcp") as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            assert "results" in text.lower() or "echo" in text.lower()
